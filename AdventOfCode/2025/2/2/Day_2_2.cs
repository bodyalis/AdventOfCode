namespace AdventOfCode._2025._2._2;
...
public static class Day_2_2
{
    ...
    public class InvalidIdsChecker
    {
        ...
        private bool IsValid(long id)
        {
            ...
            int currentSize = 1;
            while (currentSize < idString.Length / 2 + 1)
            {
                if (idString.Length % currentSize != 0)
                {
                    currentSize += 1;
                    continue;
                }

                string subString = idString.Substring(0, currentSize);    
                
                if (idString == string.Concat(Enumerable.Repeat(subString, idString.Length / currentSize)))
                {
                    return false;
                }
